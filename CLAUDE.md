...
**Writes** (`log_hours`, `create_project`, `update_project`, `create_task`, `update_task`, `create_ticket`, `update_ticket`, `attach_ticket_document`) are gated by `PO_ALLOW_WRITES=true`; read tools are always on. Projects and tasks share the `im_project` schema — a task is an `im_timesheet_task` with `parent_id` set, its name sent as `project_name`. There is no DELETE: "remove" by setting status to Deleted (82).

**Ticket attachments** are the exception to "everything goes through REST": the REST API has no file upload, so attachments are written **directly to the `intranet-filestorage` filesystem** at `<PO_TICKET_FILESTORE_PATH>/<company_path>/<project_nr>` (mirrors ]po['s `im_filestorage_ticket_path`; for tickets `project_nr == project_path`). This only works in the HTTP deploy, where the service runs as `projop` — the same OS user that runs ]po[ and owns `/web/projop/filestorage` — so files are readable by ]po[; it is gated by `PO_TICKET_FILESTORE_PATH` (unset ⇒ feature off, e.g. stdio/dev). Two entry points share the resolver+writer in `server.py`: the discoverable `attach_ticket_document` tool (`url` server-side download, or small-file `content_base64`) and the multipart endpoint `POST /mcp/tickets/{id}/attachments` for large binaries (registered via `@mcp.custom_route`, so it sits behind the same pass-through auth). ]po[ does **not** pre-create a ticket's folder — it appears on first upload — so the writer does `mkdir -p` (dirs 0775 / files 0664).
